...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	if (isset($_POST['username']) && isset($_POST['password'])) {
		...
		$username = SQLite3::escapeString(trim(filter_input(INPUT_POST, 'username', FILTER_SANITIZE_STRING)));
		$password = filter_input(INPUT_POST, 'password', FILTER_SANITIZE_STRING);
		db_exec($db, "INSERT INTO clubs VALUES(\"$username\", \"" . password_hash($password, PASSWORD_DEFAULT) . "\")");
		if (password_verify($password, db_queryUserPasswordHash($db, $username))) {
			...
			start_html();
			start_form();
			echo '<p>Internal Server Error. Please report this to STC.</p>';
			end_html();
		}
		$db->close();
		die();
